...
2. 팝업 창을 닫으려면 창 왼쪽 위의 **X**를 클릭합니다.
...
2. 페이지 이름을 **Overview**로 바꾼 다음, **Enter** 키를 누릅니다.
...
6. 이미지를 끌어 왼쪽 위에 놓고 안내선을 끌어서 크기를 조정합니다.
...
38. 시각적 개체의 서식을 지정하려면 **서식** 창을 엽니다.

    ![그림 3](Linked_image_Files/07-design-report-in-power-bi-desktop_image35.png)

39. **바**와 **색** 그룹을 차례대로 확장한 후, **기본값** 속성을 적절한 색(세로 막대형 차트/꺾은선형 차트를 보완하는 색)으로 설정합니다.
...
2. 페이지의 이름을 **Profit**으로 바꿉니다.

    ![그림 43](Linked_image_Files/07-design-report-in-power-bi-desktop_image39.png)

  

3. **Region \| Region** 필드를 활용해 슬라이서를 추가합니다.

4. **서식** 창을 사용하여 **슬라이서 설정** > **선택** 그룹의 “모두 선택” 옵션을 사용하도록 설정합니다.
...
8. 다음 5개의 Sales 테이블 필드를 **값** 웰/영역에 추가합니다.
...
9. **필터** 창(**시각화** 창 왼쪽에 위치)에서 **이 페이지의 필터** 웰/영역을 확인하세요(아래로 스크롤해야 할 수 있음).
...
12. 다음 각 **Product** 테이블 필드를 **이 페이지의 필터** 웰/영역에 추가하여 **Category** 카드 바로 아래에 각각 추가 및 축소합니다.
...
1. 새 페이지를 만든 다음, **My Performance**으로 이름을 바꿉니다.

1. 행 수준 보안 필터의 성능을 시뮬레이션하려면 **Salesperson (Performance) \| Salesperson** 필드를 필터 창의 이 페이지의 필터로 끌어옵니다.
   
    ![필터 창에 있는 Salesperson 필드의 이미지.](Linked_image_Files/07-design-report-in-power-bi-desktop_image999.png) 

1. **Michael Blythe**를 선택합니다. 이제 **My Performance** 보고서 페이지의 데이터가 Michael Blythe의 데이터만 표시하도록 필터링됩니다.
...
1. 시각적 개체의 형식을 지정합니다.
    ...
    - **설명 값** 그룹에서 **텍스트 크기** 속성을 **28pt**로 늘립니다.

    - **일반** > **효과** > **배경** 그룹에서 **색**을 연한 회색으로 설정합니다.

    ![그림 79](Linked_image_Files/07-design-report-in-power-bi-desktop_image53.png)

1. **묶인 막대형 차트** 시각적 개체를 추가한 다음 크기를 조정하고 배치하여 다중 행 카드 시각적 개체 아래에 배치하여 페이지의 나머지 높이를 채우고, 다중 행 카드 시각적 개체 너비의 절반을 채우도록 사이즈를 조정합니다.
...
1. Microsoft Edge 브라우저 창의 Power BI 서비스 내 **탐색** 창(왼쪽에 위치, 축소 가능함)에서 **내 작업 영역**을 선택하여 접속합니다.

    ![그림 93](Linked_image_Files/07-design-report-in-power-bi-desktop_image62.png)

2. 작업 영역의 내용을 검토하여 **Sales Analysis** 보고서와 데이터 세트를 확인합니다.

    *Power BI Desktop 파일을 게시할 때 데이터 모델이 데이터 세트로 게시되었습니다.*

    보고서를 열려면 **Sales Analysis** 보고서를 클릭합니다.

4. 왼쪽의 **페이지** 창에서 **Overview** 페이지를 선택합니다.

    **참고**: 맵 시각적 개체가 비활성화되었다는 오류가 표시되면 브라우저를 새로 고치세요.**

5. **Region** 슬라이서에서 **Ctrl** 키를 누르면 여러 지역을 선택합니다.

6. 세로 막대형/꺾은선형 차트에서 막대를 선택하여 페이지를 교차 필터링합니다.
...
17. 왼쪽의 **페이지** 창에서 **Profit** 페이지를 선택합니다.

    ![그림 84](Linked_image_Files/07-design-report-in-power-bi-desktop_image68.png)

18. **Region** 슬라이서에는 현재 **Overview** 페이지의 **Region** 슬라이서와는 다른 옵션이 선택되어 있습니다.
...
20. 행렬 시각적 개체에서 더하기(+) 단추를 사용하여 **Fiscal** 계층 구조로 드릴다운합니다.

21. **My Performance** 페이지를 선택합니다.
